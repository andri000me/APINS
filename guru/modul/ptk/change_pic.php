...
function changeProfilePic() {
	$post = isset($_POST) ? $_POST: array();
	$max_width = "500"; 
	$userId = isset($post['hdn-profile-id']) ? $post['hdn-profile-id'] : 0;
	$path = 'images/tmp';
...
function saveProfilePic(){
	...
	if($post['id']){
		$sql_query = "SELECT * FROM pengguna WHERE ptk_id = '".mysqli_escape_string($conn, $post['id'])."'";		
		$resultset = mysqli_query($conn, $sql_query) or die("database error:". mysqli_error($conn));		
		if(mysqli_num_rows($resultset)) {                     
			$sql_update = "UPDATE pengguna set gambar='".mysqli_escape_string($conn,$post['image_name'])."' WHERE ptk_id = '".mysqli_escape_string($conn, $post['id'])."'";			
