...
?>
    ...
    <section class="content">
		<?php if($level==98 or $level==97){ ?>
				<form class="form-horizontal" action="pas.php" method="GET">
		...
		<div class="box box-primary">
			<div class="box-header">
              <h3 class="box-title">Penilaian Akhir Semester <?=$mpm['nama_mapel'];?> Kelas <?=$romb;?></h3>
			  <div class="box-tools pull-right">
				<?php if($jw>0){ ?>
				<form role="form" action="pas.php" method="GET">
				<div class="form-group">
					<input type="hidden" name="kelas" class="form-control" value="<?php echo $romb;?>">
					<input type="hidden" name="tapel" class="form-control" value="<?php echo $tapel;?>">
					<input type="hidden" name="smt" class="form-control" value="<?php echo $smt;?>">
...
<script type="text/javascript" language="javascript" class="init">
	...
	$(document).ready(function() {
		nK3 = $("#nilaiPTS").DataTable({
				"searching": false,
				"ajax": "../modul/semester/dataAS.php?kelas=<?=$romb;?>&tapel=<?=$tapel;?>&smt=<?=$smt;?>&mp=<?=$mpid;?>&peta=3&kd=<?=$kd;?>",
				"order": []
			});
	});
